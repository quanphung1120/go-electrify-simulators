...
```typescript
// packages/shared-types/src/tests/validation.test.ts
import {
  validateIncomingMessage,
  ablyStartSessionSchema,
  ABLY_EVENTS,
...
describe("Message Validation with Transforms", () => {
  it("should handle different targetSOC naming variations", () => {
    // Test camelCase
    const camelCaseData = { targetSOC: 80 };
    const result1 = validateIncomingMessage(
      ablyStartSessionSchema,
      camelCaseData,
      ABLY_EVENTS.START_SESSION
    );
    expect(result1.targetSOC).toBe(80);

    // Test snake_case
    const snakeCaseData = { target_soc: 85 };
    const result2 = validateIncomingMessage(
      ablyStartSessionSchema,
      snakeCaseData,
      ABLY_EVENTS.START_SESSION
    );
    expect(result2.targetSOC).toBe(85);

    // Test PascalCase
    const pascalCaseData = { TargetSOC: 90 };
    const result3 = validateIncomingMessage(
      ablyStartSessionSchema,
      pascalCaseData,
      ABLY_EVENTS.START_SESSION
    );
    expect(result3.targetSOC).toBe(90);
  });

  it("should reject invalid data", () => {
    const invalidData = { targetSOC: -10 };
    expect(() =>
      validateIncomingMessage(
        ablyStartSessionSchema,
        invalidData,
